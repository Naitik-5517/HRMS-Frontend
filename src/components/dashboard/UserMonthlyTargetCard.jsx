            ...
            <div className="flex items-center gap-4">
              <div className="w-80 relative">
                <DatePicker.RangePicker
                  value={dateRanges[month.key]}
                  onChange={range => handleRangeChange(month.key, range)}
                  disabledDate={current =>
                    !current ||
                    current < month.range[0] ||
                    current > month.range[1]
                  }
                  allowClear={false}
                  format="DD-MM-YYYY"
                  className="w-full rounded-lg border-blue-300 shadow-sm focus:border-blue-500"
                  getPopupContainer={trigger => trigger.parentNode}
                  style={{ background: "#f8fafc" }}
                />
                <style>{`
                  .ant-picker-panels > *:nth-child(2) { display: none !important; }
                `}</style>
              </div>
              <button
                className="ml-2 flex items-center gap-1 px-3 py-2 rounded-md bg-green-50 hover:bg-green-100 border border-green-200 shadow-sm transition text-green-700 font-medium"
                title="Export to Excel"
                aria-label="Export to Excel"
                // onClick={handleExportExcel} // Add export logic here
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" fill="#22c55e"/><path d="M8 8l8 8M16 8l-8 8" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                ...
                <tbody className="bg-white divide-y divide-blue-50">
                  {month.agents.map((agent) => (
                    <tr key={agent.id} className="hover:bg-blue-50 transition group">
                      {/* User Name */}
                      <td
                        className="px-6 py-3 text-black font-medium whitespace-nowrap cursor-pointer"
                        onDoubleClick={() => handleCellDoubleClick(month.key, agent.id, 'userName', agent.userName)}
                      >
                        {editingCell.monthKey === month.key && editingCell.agentId === agent.id && editingCell.field === 'userName' ? (
                          <input
                            className="border border-blue-300 rounded px-2 py-1 w-full text-black bg-white focus:outline-none focus:ring-2 focus:ring-blue-200"
                            value={editState[month.key]?.[agent.id]?.userName ?? agent.userName}
                            autoFocus
                            onChange={e => handleEditChange(e, month.key, agent.id, 'userName')}
                            onBlur={() => handleEditSave(month.key, agent.id, 'userName')}
                            onKeyDown={e => { if (e.key === 'Enter') handleEditSave(month.key, agent.id, 'userName'); }}
                          />
                        ) : (
                          agent.userName
                        )}
                      ...
                      <td
                        className="px-6 py-3 text-center text-black cursor-pointer"
                        onDoubleClick={() => handleCellDoubleClick(month.key, agent.id, 'dailyRequiredHours', agent.dailyRequiredHours)}
                      >
                        {editingCell.monthKey === month.key && editingCell.agentId === agent.id && editingCell.field === 'dailyRequiredHours' ? (
                          <input
                            type="number"
                            className="border border-blue-300 rounded px-2 py-1 w-16 text-black bg-white focus:outline-none focus:ring-2 focus:ring-blue-200 text-center"
                            value={editState[month.key]?.[agent.id]?.dailyRequiredHours ?? agent.dailyRequiredHours}
                            autoFocus
                            onChange={e => handleEditChange(e, month.key, agent.id, 'dailyRequiredHours')}
                            onBlur={() => handleEditSave(month.key, agent.id, 'dailyRequiredHours')}
                            onKeyDown={e => { if (e.key === 'Enter') handleEditSave(month.key, agent.id, 'dailyRequiredHours'); }}
                          />
                        ) : (
                      ...
                      <td className="px-6 py-3 text-center cursor-pointer">
                        {editingCell.monthKey === month.key && editingCell.agentId === agent.id && editingCell.field === 'achievedTotal' ? (
                          <>
                            <input
                              type="number"
                              className="border border-blue-300 rounded px-2 py-1 w-16 text-black bg-white focus:outline-none focus:ring-2 focus:ring-blue-200 text-center mr-1"
                              value={editState[month.key]?.[agent.id]?.monthlyAchievedTarget ?? agent.monthlyAchievedTarget}
                              autoFocus
                              onChange={e => handleEditChange(e, month.key, agent.id, 'monthlyAchievedTarget')}
                              onBlur={() => handleEditSave(month.key, agent.id, 'achievedTotal')}
                              onKeyDown={e => { if (e.key === 'Enter') handleEditSave(month.key, agent.id, 'achievedTotal'); }}
                            />
                            <span className="mx-1 text-black">/</span>
                            <input
                              type="number"
                              className="border border-blue-300 rounded px-2 py-1 w-16 text-black bg-white focus:outline-none focus:ring-2 focus:ring-blue-200 text-center"
                              value={editState[month.key]?.[agent.id]?.monthlyTotalTarget ?? agent.monthlyTotalTarget}
                              onChange={e => handleEditChange(e, month.key, agent.id, 'monthlyTotalTarget')}
                              onBlur={() => handleEditSave(month.key, agent.id, 'achievedTotal')}
                              onKeyDown={e => { if (e.key === 'Enter') handleEditSave(month.key, agent.id, 'achievedTotal'); }}
                            />
                          </>
                          ...
                          <span
                            onDoubleClick={() => handleCellDoubleClick(month.key, agent.id, 'achievedTotal', null)}
                          >
