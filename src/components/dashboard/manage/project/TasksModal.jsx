...
import { X, Plus, Loader2, ChevronDown, Pencil, Trash2, Upload, FileSpreadsheet } from 'lucide-react';
...
const TasksModal = ({
  ...
  }, [showTeamDropdown, showColumnDropdown]);
  ...
  const handleAddTask = async () => {
    ...
    if (success) {
      setFormData({ name: '', description: '', target: '', teamIds: [], excelFile: null, selectedColumns: [] });
      setShowTeamDropdown(false);
      setColumnNames([]);
      setShowColumnDropdown(false);
      if (fileInputRef.current) {
